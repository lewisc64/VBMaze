...
Namespace VBGame
    ...
    Public Class Images
        ...
        ''' <remarks></remarks>
        Public Shared Sub save(image As Bitmap, path As String, Optional format As System.Drawing.Imaging.ImageFormat = Nothing)
    ...
    Public Class Collisions
        ...
        Public Shared Function rect(r1 As Rectangle, r2 As Rectangle) As Boolean
    ...
    Public Class MouseEvent
        ...
        Public Sub New(location As Point, action As Byte, button As Byte)
            Me.action = action
    ...
    Public Class DrawBase
        ...
        Sub drawCircle(circle As Circle, color As System.Drawing.Color, Optional filled As Boolean = True)
            drawEllipse(Collisions.circleToRect(shiftCircle(circle)), color, filled) 'Rect shift not needed, ellipse takes care of that.
        End Sub
    ...
    Public Class Display
        ...
        ''' <remarks></remarks>
        Public Sub New(ByRef f As Form, resolution As Size, Optional title As String = "", Optional sharppixels As Boolean = False, Optional fullscreen As Boolean = False)
            form = f
            ...
            form.Invoke(Sub() form.GetType().GetProperty("DoubleBuffered", BindingFlags.Instance Or BindingFlags.Public Or BindingFlags.NonPublic).SetValue(form, True))
    ...
    Public Class Surface
        ...
        ''' <remarks></remarks>
        Public Sub New(rect As Rectangle, parentdisplay As Display)
    ...
    Public Class Sound
        ...
        Private Function GetMCIErrorString(ByVal ErrorCode As Integer) As String
            Dim s As String = Space(1024)
            mciGetErrorString(ErrorCode, s, Len(s))
            Return Trim(s)
        End Function
        ...
        Public Sub New(filename As String)
            ...
            name = "sound_" & CStr(nextID)
            If filename.Contains(".mp3") Then
                name = getPath()
            End If
            nextID += 1
            load()
    ...
    Public Class Animation
        ...
        Sub getFramesFromStrip(strip As Image, rowcolumn As Size, Optional nframes As Integer = 0, Optional reverse As Boolean = False)
            frames = Images.sliceSpriteSheet(strip, rowcolumn, nframes, reverse)
        End Sub
    ...
    Public Class Sprite
        ...
        Public Overridable Sub draw(Display As Display)
            Display.drawRect(getRect(), color)
        End Sub

        Public Sub move(Optional trig As Boolean = False)
            Dim mp As PointF
            mp = calcMove(trig)
            x = mp.X
            y = mp.Y
        ...
        Public Overridable Function calcMove(Optional trig As Boolean = False) As PointF
            Dim xt, yt As Double
            If trig Then
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Overridable Function keepInBounds(bounds As Rectangle, Optional trig As Boolean = False, Optional bounce As Boolean = False) As Boolean
        ...
        Overridable Function keepOutsideBounds(bounds As Rectangle, Optional trig As Boolean = False, Optional bounce As Boolean = False) As Byte
            If Collisions.rect(getRect(), bounds) Then
                Dim side As Byte
    ...
    Class Button
        ...
        ''' <remarks></remarks>
        Public Sub New(ByRef display As Display, text As String, Optional rect As Rectangle = Nothing, Optional fontname As String = "Arial", Optional fontsize As Integer = 0)
            Me.display = display
            If Not IsNothing(rect) Then
                setRect(rect)
            End If
        ...
        Public Function handle(e As MouseEvent) As Byte
            If Collisions.rect(New Rectangle(e.location.X, e.location.Y, 1, 1), getRect()) Then
                hover = True
    ...
    Class TextInput
        ...
        Public Sub New(rect As Rectangle, Optional fontname As String = "Arial", Optional fontsize As Integer = 0)
        ...
        Public Overridable Sub draw(mdisplay As Display)
    ...
    Class SimpleFont
        ...
        Public Sub New(sheet As Image)
            ...
            characters = Images.sliceSpriteSheet(image, New Size(10, 4))
